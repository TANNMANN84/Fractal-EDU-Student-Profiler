...
const TermPill: React.FC<{ term: Term; activeTerm: Term; onClick: (term: Term) => void }> = ({ term, activeTerm, onClick }) => (
    <button
        ...
        className={`px-4 py-2 font-semibold text-sm rounded-md transition-all duration-200 ${
            activeTerm === term ? 'bg-indigo-600 text-white shadow' : 'bg-white text-gray-700 hover:bg-gray-100 border'
        }`}
    >
        Term {term}
    </button>
...
const JuniorMonitoring: React.FC<JuniorMonitoringProps> = ({ monitoringDoc, onSave, students, className, classData }) => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow flex-wrap gap-4">
                <div>
                     <h2 className="text-2xl font-bold text-gray-800">Junior Monitoring: {className}</h2>
                     <p className="text-gray-600">Complete all required monitoring tasks for this class.</p>
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => onSave(doc)} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold transition-colors">Save Changes</button>
                    <button onClick={handleExportForReview} className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-semibold transition-colors">Export for Review</button>
                </div>
            </div>
            ...
            <Section title="1. Syllabus Implementation & Reporting">
                 ...
                 <div className="flex items-center">
                    <input type="checkbox" id="certifySyllabus" checked={doc.certifySyllabus} onChange={e => handleDocChange('certifySyllabus', e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                    <label htmlFor="certifySyllabus" className="ml-2 text-sm font-medium text-gray-900">I certify that all syllabus outcomes have been taught.</label>
                </div>
                 <div className="border-t pt-4">
                    <h4 className="font-semibold text-gray-700 mb-2">Term-based Documents</h4>
                    ...
                    <div className="space-y-4 p-4 bg-gray-50 border rounded-md">
                        ...
                        {doc.teachingPrograms[activeTerm]?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleMultiFileRemove(`teachingPrograms.${activeTerm}`, i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
            ...
            <Section title="2. Assessment Documents">
                ...
                <div>
                    ...
                    {doc.assessmentTask1?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4 mt-1 bg-gray-50 p-1 rounded"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleAssessmentFileRemove('assessmentTask1', i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
                </div>
                <div>
                    ...
                    {doc.assessmentTask2?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4 mt-1 bg-gray-50 p-1 rounded"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleAssessmentFileRemove('assessmentTask2', i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
                </div>
                 <div>
                    ...
                    {doc.assessmentTask3?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4 mt-1 bg-gray-50 p-1 rounded"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleAssessmentFileRemove('assessmentTask3', i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
                </div>
                 <div>
                    ...
                    {doc.prePostDiagnostic?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4 mt-1 bg-gray-50 p-1 rounded"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleAssessmentFileRemove('prePostDiagnostic', i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
                </div>
            </Section>
            ...
            <Section title="3. Student Performance & Achievement">
                 <div className="border-t pt-4">
                    ...
                    <div className="space-y-4 p-4 bg-gray-50 border rounded-md">
                         ...
                         <div className="flex items-center">
                            <input type="checkbox" id="specificLearningNeeds" checked={doc.specificLearningNeeds[activeTerm]} onChange={e => handleDocChange(`specificLearningNeeds.${activeTerm}`, e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                            <label htmlFor="specificLearningNeeds" className="ml-2 text-sm font-medium text-gray-900">Specific learning needs of students have been addressed this term.</label>
                        </div>
                    </div>
                 </div>
            </Section>
            ...
            <Section title="4. Records of Communication">
                 <div className="border-t pt-4">
                    ...
                    <div className="space-y-4 p-4 bg-gray-50 border rounded-md">
                         <div>
                            <h5 className="font-medium text-gray-800">Students Causing Concern</h5>
                            <button onClick={() => setIsAddConcernModalOpen(true)} className="text-sm bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700 mt-1">Log Concern</button>
                            <div className="mt-2 space-y-1">
                                {doc.studentsCausingConcern[activeTerm].map(concern => (
                                    <div key={concern.id} className="flex justify-between items-center text-sm p-1 bg-white border rounded">
                                        <button onClick={() => storageService.triggerDownload(concern.file)} className="text-blue-600 hover:underline">{concern.file.name}</button>
                        ...
                        <div>
                            <h5 className="font-medium text-gray-800">Illness/Misadventure</h5>
                             <MonitoringFileUpload label="" file={null} onUpload={(file) => handleMultiFileUpload(`illnessMisadventure.${activeTerm}`, file)} onRemove={() => {}} />
                             {doc.illnessMisadventure[activeTerm]?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleMultiFileRemove(`illnessMisadventure.${activeTerm}`, i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
                        </div>
                         <div>
                            <h5 className="font-medium text-gray-800">Malpractice</h5>
                            <MonitoringFileUpload label="" file={null} onUpload={(file) => handleMultiFileUpload(`malpractice.${activeTerm}`, file)} onRemove={() => {}} />
                            {doc.malpractice[activeTerm]?.map((f, i) => <div key={i} className="flex items-center justify-between text-sm ml-4"><button onClick={() => storageService.triggerDownload(f)} className="text-blue-600 hover:underline">{f.name}</button><button onClick={() => handleMultiFileRemove(`malpractice.${activeTerm}`, i)} className="text-red-500 hover:text-red-700 font-bold">&times;</button></div>)}
                        </div>
                    </div>
                 </div>
            </Section>
            ...
            <Section title="5. Sign Off">
                <p className="text-sm text-gray-600">Please complete and sign off for the relevant term.</p>
                <div className="flex flex-wrap gap-2 mb-4">
                    {(['1', '2', '3', '4'] as Term[]).map(t => <TermPill key={t} term={t} activeTerm={activeTerm} onClick={setActiveTerm} />)}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 border rounded-md">
                    <div className="text-center">
                        <h4 className="font-semibold">Teacher Sign Off</h4>
                        {teacherSignOff?.date ? (
                            ...
                            <button onClick={() => { setSignerRole('teacher'); setIsSignOffModalOpen(true); }} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 font-semibold">Sign as Teacher</button>
                        )}
                    </div>
                    <div className="text-center">
                        <h4 className="font-semibold">Head Teacher Sign Off</h4>
                        {headTeacherSignOff?.date ? (
                            ...
                            <div className="mt-2 p-2 bg-yellow-100 border border-yellow-300 rounded text-sm text-yellow-800">Awaiting Signature</div>
                        )}
                    </div>
                </div>
