...
const ReportSection: React.FC<{ title: string; children: React.ReactNode; condition?: boolean }> = ({ title, children, condition = true }) => {
    ...
    return (
        <div className="mt-4">
            <h4 className="text-base font-bold text-gray-800 border-b-2 border-gray-300 pb-1 mb-2">{title}</h4>
            {children}
        </div>
    );
};
...
const ReportView: React.FC<ReportViewProps> = ({ students, options, onClose }) => {
    ...
    return (
        <dialog ref={dialogRef} onClose={onClose} className="p-0 rounded-lg shadow-xl w-11/12 max-w-5xl backdrop:bg-black backdrop:opacity-50 border border-gray-300">
            <style>{`
                @media print {
            ...
            `}</style>
             <div className="flex justify-between items-center p-4 bg-gray-100 border-b border-gray-200 sticky top-0 z-10 no-print">
                <h2 className="text-xl font-bold text-gray-900">Generated Student Report</h2>
                <div className="flex items-center gap-4">
                    <button onClick={handlePrint} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 font-semibold">Print Report</button>
                    <button onClick={handleClose} className="text-2xl font-light text-gray-600 hover:text-gray-900 leading-none">&times;</button>
                </div>
            </div>
            <div className="p-2 md:p-6 bg-white text-gray-900 print-area" style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                         ...
                         <div className="p-4 border border-gray-300 rounded-lg">
                            ...
                            <ReportSection title="NAPLAN Results" condition={options.academicNaplan}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <h5 className="font-semibold">Year 7</h5>
                                    ...
                                    {student.profile.currentYearGroup >= 9 && (
                                        <div>
                                            <h5 className="font-semibold">Year 9</h5>
                                            <ul className="list-disc list-inside">
                                ...
                                <table className="w-full border-collapse border border-gray-300 text-xs">
                                    <thead className="bg-gray-100">
                                        <tr>
                                            ...
                                            <th className="border p-1 text-left">Adjustments</th>
                                    ...
                                    {student.evidenceLog?.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(log => (
                                        <tr key={log.logId}>
                                            <td className="border p-1 align-top w-1/12">{new Date(log.date).toLocaleDateString('en-AU')}</td>
                                            <td className="border p-1 align-top w-6/12 whitespace-pre-wrap">{log.note}</td>
                                            <td className="border p-1 align-top w-3/12">{log.adjustments_used.join(', ')}</td>
                                            <td className="border p-1 align-top w-2/12">{log.adjustment_level}</td>
                                        </tr>
                                    ))}
                                    </tbody>
                                </table>
                                {(!student.evidenceLog || student.evidenceLog.length === 0) && <p className="text-sm text-gray-500 mt-1">No entries.</p>}
                            </ReportSection>
             ...
             <div className="p-4 bg-gray-100 border-t flex justify-end no-print">
                 <button onClick={handleClose} className="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 font-semibold transition-colors">Close</button>
            </div>
        </dialog>
    );
};
