...
const VIEW_MODES = ["Default Grid", "List View", "Academic View", "Wellbeing/Plans View", "HPGE Profile View"];
...
const ClassView: React.FC<ClassViewProps> = ({ classData }) => {
  ...
  const [reportToShow, setReportToShow] = useState<{ students: Student[], options: any } | null>(null);
  ...
  const handleGenerateReport = (selectedStudentIds: string[], options: any) => {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {studentsInClass.map(student => (
                    <StudentCard
                      ...
                      onRemoveFromClass={() => handleRemoveStudentFromClass(student.studentId)}
                    />
                  ))}
                </div>
            );
        case 'List View':
            return <StudentListView students={studentsInClass} onSelectStudent={setSelectedStudent} />;
            ...
            <div className="flex justify-between items-start mb-4 pb-2 flex-wrap gap-4">
                <div>
                    <div className="flex items-center gap-4 flex-wrap">
                        <div>
                            <label htmlFor="view-mode-select" className="text-sm font-medium text-gray-700 mr-2">View Mode:</label>
                            <select id="view-mode-select" value={viewMode} onChange={handleViewModeChange} className="p-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
                                {VIEW_MODES.map(mode => <option key={mode} value={mode}>{mode}</option>)}
                            </select>
                        </div>
                        {viewMode !== 'Default Grid' && viewMode !== 'List View' && (
                            <div>
                                <label htmlFor="data-point-select" className="text-sm font-medium text-gray-700 mr-2">Data Point:</label>
                                <select id="data-point-select" value={dataPoint || ''} onChange={e => setDataPoint(e.target.value)} className="p-1 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm">
      ...
      {activeTab === 'Reports and Analysis' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1">
                 <div className="bg-white p-6 rounded-lg shadow h-full flex flex-col items-center justify-center text-center">
                    <h3 className="text-xl font-bold text-gray-800">Report Generation</h3>
                    <p className="text-gray-600 mt-2">Generate detailed PDF reports for individual students or the entire class.</p>
                    <button
                        onClick={() => setIsReportGeneratorOpen(true)}
                        className="mt-4 bg-green-600 text-white px-5 py-2 rounded-md hover:bg-green-700 font-semibold transition-colors"
                    >
